(function() {
  ...
  directive('begriffs.pagination', function () {
      var defaultLinkGroupSize = 3, defaultClientLimit = 250, defaultPerPage = 16;
          ...
          $scope.updatePresets  = function () {
            if($scope.autoPresets) {
              ...
              for(i = Math.min(3, quantizedIndex($scope.perPage || defaultPerPage));
                  ...
                  i++) {
//console.log(quantizedIndex($scope.perPage||defaultPerPage));
                presets.push(quantizedNumber(i));
              }
//console.log($scope.myPerPageSets);
              if($scope.myPerPageSets){
	          presets = $scope.myPerPageSets;
              }
              $scope.perPagePresets = [16,32,48];
